  ...
  return (
    <div className="bg-[#120620] border-2 border-cyan-400 p-4 sm:p-6 relative overflow-hidden shadow-[0_0_20px_rgba(34,211,238,0.15)] w-full">
      ...
      <div className="mb-6 flex flex-col sm:flex-row sm:items-center justify-between gap-3">
        ...
        <span className="text-[9px] sm:text-[10px] font-mono text-pink-500 font-bold border border-pink-500/30 px-2 py-0.5 bg-pink-950/20 self-start sm:self-auto">
          TOTAL_RECORDS: {analyses.length}
        </span>
      </div>

      {/* Responsive Table Wrap with Touch-Friendly Overflow mechanics */}
      <div className="overflow-x-auto w-full border border-pink-500/20 scrollbar-thin scrollbar-thumb-cyan-500 scrollbar-track-[#1b0a2e] -mx-2 sm:mx-0">
        <table className="w-full border-collapse font-mono text-xs text-left min-w-[600px]">
          <thead>
            <tr className="bg-[#1b0a2e] border-b-2 border-pink-500 text-pink-400 font-black tracking-wider uppercase">
              ...
              <th className="p-3 min-w-[200px]">VENTURE ASSET DESCRIPTION</th>
              <th className="p-3 text-center w-20">SCORE</th>
              <th className="p-3 min-w-[150px]">CORE DIRECTIVE</th>
          ...
          <tbody className="divide-y divide-pink-950/50">
            {analyses.length === 0 ? (
              ...
              analyses.map((analysis, index) => (
                ...
                >
                  {/* ID Column */}
                  <td className="p-3 text-center text-pink-500 font-bold border-r border-pink-950/30">
                    #{analysis.id}
                  </td>

                  {/* Idea/Description */}
                  <td className="p-3 text-slate-200 font-sans max-w-xs md:max-w-md truncate group-hover:text-cyan-200 transition-colors">
                    {analysis.idea}
                  </td>

                  {/* Score Column with Dynamic Neon Intensity */}
                  <td className="p-3 text-center border-x border-pink-950/30">
                  ...
                  <td className="p-3 text-slate-400 font-medium truncate max-w-[150px] md:max-w-xs">
                    {analysis.recommendation}
                  </td>

                  {/* Report Export Button */}
                  <td className="p-3 text-right border-l border-pink-950/30">
                    <a
                      href={`http://127.0.0.1:8000/export-report/${analysis.id}`}
                      target="_blank"
                      rel="noopener noreferrer"
                      className="inline-block bg-[#1a0c2c] border border-cyan-500/40 hover:border-cyan-400 text-cyan-400 hover:text-black hover:bg-cyan-400 font-black px-2.5 py-1 text-[10px] tracking-wider uppercase skew-x-[-8deg] transition-all whitespace-nowrap"
                    >
      ...
      <div className="mt-4 flex flex-wrap justify-between items-center gap-2 text-[9px] font-mono text-cyan-500/40 tracking-widest">
